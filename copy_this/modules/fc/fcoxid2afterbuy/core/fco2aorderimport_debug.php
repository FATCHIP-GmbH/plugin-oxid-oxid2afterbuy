...
class fco2aorderimport_debug extends fco2abase {
    ...
    protected function _fcGetOrderDeliveryData($oOrder, $oAddress) {
        ...
        $oOrder->oxorder__oxdeladdinfo = $oAddress->oxaddress__oxaddinfo;
    ...
    protected function _fcGetOrderBillData($oOrder, $oUser) {
        ...
        $oOrder->oxorder__oxbilladdinfo = $oUser->oxuser__oxaddinfo;
    ...
    protected function _fcGetUserData($oBillingAddress, $oUser) {
        ...
        $oUser->oxuser__oxaddinfo = new oxField($aStreetParts['addinfo']);
    ...
    protected function _fcSetUserAddressData($oShippingAddress, $oUser) {
        ...
        $oAddress->oxaddress__oxaddinfo = new oxField($aStreetParts['addinfo']);
    ...
    protected function _fcpoSplitStreetAndStreetNr($sStreetAndStreetNr) {
        $aReturn = array();
        if (strpos($sStreetAndStreetNr, "Packstation") !== false) { // try to extract Packstation details
            $aExplode = explode(" ", $sStreetAndStreetNr);
            $sPostnummer = false;
            $sPackstationNummer = false;
            foreach ($aExplode as $sItem) {
                if (is_numeric($sItem)) {
                    if (strlen($sItem) >= 6) {
                        $sPostnummer = $sItem;
                    } else {
                        $sPackstationNummer = $sItem;
                    }
                }
            }
            if ($sPostnummer !== false && $sPackstationNummer !== false) {
                $aReturn['street'] = "Packstation";
                $aReturn['streetnr'] = $sPackstationNummer;
                $aReturn['addinfo'] = $sPostnummer;
            }
        }
        if (empty($aReturn)) { // try regex splitting
            ...
            $aReturn['street'] = $sStreet;
            $aReturn['streetnr'] = $sStreetNr;
            $aReturn['addinfo'] = "";
        }
        return $aReturn;
    }
